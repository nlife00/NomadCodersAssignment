...
input_guardrail_agent = Agent(
    ...
    instructions="""
    Ensure the user's request is related to a restaurant reservation, order, menu inquiry, food allergy, or food ingredient inquiry. Avoid responding to requests that are off-topic. If the request is off-topic, clearly state the reason for the request. While it's okay to engage in a brief conversation, especially at the beginning of the conversation, avoid responding to unrelated requests.
    """,
...
def dynamic_triage_agent_instructions(wrapper: RunContextWrapper, agent: Agent[UserAccountContext]):
    ...
    당신은 레스토랑의 고객 지원 에이전트입니다. 당신은 고객의 예약, 메뉴조회, 음식 알레르기, 주문 및 주문 확인과 같은 업무만 답변 할 수 있습니다. 
